...
public class MQTTServicePublish extends Service implements MqttCallback, GoogleApiClient.ConnectionCallbacks,
    ...
    private static final String NOTIF = "Notification_LOG";
    private static final String REPORT = "Publish_LOG";
    ...
    private String longlat = "{\"topiName\": \"anon\"," +
            "\"timeStamp\": \"0L\"," +
            "\"longitude\": \"0.0\"," +
            "\"latitude\": \"0.0\"}";
    ...
    public static final String MQTT_MSG_RECEIVED_INTENT = "pubsub.mqtt.android.MSGRECVD";
    public static final String MQTT_MSG_RECEIVED_TOPIC = "pubsub.mqtt.android.MSGRECVD_TOPIC";
    public static final String MQTT_MSG_RECEIVED_MSG = "pubsub.mqtt.android.MSGRECVD_MSGBODY";

    // constants used to tell the Activity UI the connection status
    public static final String MQTT_STATUS_INTENT = "pubsub.mqtt.android.STATUS";
    public static final String MQTT_STATUS_MSG = "pubsub.mqtt.android.STATUS_MSG";

    // constant used internally to schedule the next ping event
    public static final String MQTT_PING_ACTION = "pubsub.mqtt.android.PING";
...
//                Log.e(REPORT, "00: publish to reconnect");
            }
        }
    ...
    @Override
    public void messageArrived(String topic, MqttMessage payloadbytes) throws Exception {
        // we protect against the phone switching off while we're doing this
        //  by requesting a wake lock - we request the minimum possible wake
        //  lock - just enough to keep the CPU running until we've finished
        PowerManager pm = (PowerManager) getSystemService(POWER_SERVICE);
        ...
        String messageBody = new String(payloadbytes.getPayload());
...
//            notifyUser("Data Sent", topic, messageBody);
            Log.e(NOTIF, "Data Sent: "+topic+" "+messageBody);
...
//            String time = "" + cl.get(Calendar.HOUR_OF_DAY) + ":" + cl.get(Calendar.MINUTE) + ":" + cl.get(Calendar.SECOND);
            ...
            longlat = "{\"topiName\": \""+topicName+"\"," +
                    "\"timeStamp\" : \""+String.valueOf(timestamp)+"\"," +
                    "\"longitude\": \""+String.valueOf(location.getLongitude())+"\"," +
                    "\"latitude\": \""+String.valueOf(location.getLatitude())+"\"}";

            Log.e(TAG, "position: " + location.getLatitude() + ", " + location.getLongitude() + " accuracy: " + location.getAccuracy() + " timeStamp: "+timestamp);
...
//            }
    ...
    private MemoryPersistence usePersistence = null;
    ...
    private short keepAliveSecondsScheduleNextPing = 1;
    ...
    public void onCreate() {
        ...
        mBinder = new LocalBinder<MQTTServicePublish>(this);
    ...
    public int onStartCommand(final Intent intent, int flags, final int startId) {
        ...
        System.out.println("ONCOMMAND SIRE");
    ...
    synchronized void handleStart(Intent intent, int startId) {
        ...
        if (isAlreadyConnected() == false) {
            ...
            Notification.Builder builder = new Notification.Builder(MQTTServicePublish.this);
            ...
            if (isOnline()) {
                ...
                if (connectToBroker()) {
                    Log.e(REPORT, "01: publish to start");
                    // we publish to a topic - registering to receive push
                    //  notifications with a particular key
                    // in a 'real' app, you might want to publish to multiple
    ...
    private void notifyUser(String alert, String title, String body) {
        ...
        nm.notify(MQTT_NOTIFICATION_UPDATE, builder.build());
    }
    ...
    private LocalBinder<MQTTServicePublish> mBinder;
...
//        {
        // define the connection to the broker
        mqttClient = new MqttAndroidClient(this, mqttConnSpec, generateClientId(), usePersistence);
...
//                    publishToTopic(topicName);
                    Log.e(REPORT, "02: publish on success of connectedToBroker");
                    //
                ...
                public void onFailure(IMqttToken iMqttToken, Throwable throwable) {
                    ...
                    if (firstTime){
                        notifyUser("Unable to publish", "MQTT", "Connection to broker failed");
                        firstTime = false;
                    }
     ...
     */
    private void publishToTopic(String topicName) {
        boolean published = true;

        if (isAlreadyConnected() == false) {
            published = false;
            firstTime = true;
            // quick sanity check - don't try and publish if we
            //  don't have a connection

            Log.e("mqtt", "Unable to publish as we are not connected");
        } else {
            try {
                MqttMessage message = new MqttMessage(longlat.getBytes());
                message.setQos(1);
                message.setRetained(true);
                mqttClient.publish(topicName, message);

                published = true;
                if((firstTime==false)&&(published==true)){
                    firstTime = true;
                }


                Log.e(REPORT, "06: publishToTopic succeed");
                if(firstTime){
                    notifyUser("Publish succeed", "MQTT", "Currently publishing position of "+topicName);
                    firstTime = false;
                }
        ...
        if (published == false) {
            //
            // inform the app of the failure to publish so that the UI can
            //  display an error
            broadcastServiceStatus("Unable to publish");
            ...
            // inform the user (for times when the Activity UI isn't running)
            if(firstTime){
                notifyUser("Unable to publish", "Disconnected from Broker", "Attempting to reconnect . . .");
                firstTime = false;
            }
        }
    }
    ...
    private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context ctx, Intent intent) {
            ...
            if (isOnline()) {
                ...
                if (connectToBroker()) {
                    Log.e(REPORT, "03: publish on receive of wake lock");
                    // we publish to a topic - registering to receive push
                    //  notifications with a particular key
                    publishToTopic(topicName);
    ...
    private void scheduleNextPing() {
        ...
        wakeUpTime.add(Calendar.SECOND, keepAliveSecondsScheduleNextPing);
...
//                mqttClient.publish(topicName, message);
                publishToTopic(topicName);
//                Log.e(REPORT, "05: publish onReceive of PingSender");
//            }
//            catch (MqttException e)
//            {
...
//                if (connectToBroker()) {
//                    publishToTopic(topicName);
//                    Log.e(REPORT, "04: publish to onPingSender reconnect attempt");
//                }
//            }
